...
function createManager(config) {
  ...
  async function connect() {
    ...
    // Baileys signal logger shared across socket lifecycle. "warn" surfaces
    // PreKeyError / Bad MAC / Counter errors needed to diagnose decrypt
    // failures while staying quiet in steady state. Override via env.
    ...
    sock = makeWASocket({
      ...
      auth: {
        creds: state.creds,
        // makeCacheableSignalKeyStore avoids losing key coherence between
        // assertSessions + sendMessage, fixing "Invalid PreKey ID" floods
        // and stale-session collisions caused by uncached file-store reads.
        keys: makeCacheableSignalKeyStore(state.keys, baileysLogger),
      },
      logger: baileysLogger,
      // printQRInTerminal removed: deprecated in v7. QR is rendered manually
      // in the connection.update handler below.
    });
